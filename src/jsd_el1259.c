...
#define JSD_EL1259_ENABLE_MANUAL_OP_BIT (0x01u << 3)     // 0x70n1:04 in ctrl byte

// Minimal "plain digital-IO" PDO assignment. Each channel exposes one
// non-oversampled per-channel PDO; consecutive channels are 0x04 apart.
...
#define JSD_EL1259_DI_PDO_BASE (uint16_t)0x1A0B       // "MTI Inputs 1x" Ch.1
#define JSD_EL1259_DO_PDO_BASE (uint16_t)0x1603       // "MTO Outputs 1x" Ch.1
#define JSD_EL1259_PDO_CHANNEL_STRIDE (uint16_t)0x0004
...
void jsd_el1259_read(jsd_t* self, uint16_t slave_id) {
  ...
  for (int ch = 0; ch < JSD_EL1259_NUM_DI_CHANNELS; ++ch) {
    state->input[ch] =
        (uint8_t)((txpdo->channel[ch].status & JSD_EL1259_INPUT_STATE_BIT) != 0);
  }
}
...
void jsd_el1259_process(jsd_t* self, uint16_t slave_id) {
  ...
  for (int ch = 0; ch < JSD_EL1259_NUM_DO_CHANNELS; ++ch) {
    // Always request manual operation; set the manual output level bit.
    uint8_t ctrl = JSD_EL1259_ENABLE_MANUAL_OP_BIT;
    if (state->output[ch] > 0) {
      ctrl |= JSD_EL1259_MANUAL_OUTPUT_STATE_BIT;
    }
    rxpdo->channel[ch].ctrl = ctrl;
  }
}
...
// Reassign a Sync Manager's PDOs to a contiguous per-channel set, written
// subindex-by-subindex (the EL125x family is sensitive to assignment order).
static int jsd_el1259_assign_pdos(ecx_contextt* ecx_context, uint16_t slave_id,
                                  uint16_t assign_sdo, uint16_t pdo_base,
  ...
  for (uint8_t ch = 0; ch < num_channels; ++ch) {
    uint16_t pdo_index =
        (uint16_t)(pdo_base + JSD_EL1259_PDO_CHANNEL_STRIDE * ch);
    if (!jsd_sdo_set_param_blocking(ecx_context, slave_id, assign_sdo,
                                    (uint8_t)(ch + 1), JSD_SDO_DATA_U16,
                                    &pdo_index)) {
      return 0;
    }
  }
  if (!jsd_sdo_set_param_blocking(ecx_context, slave_id, assign_sdo, 0x00,
                                  JSD_SDO_DATA_U8, &num_channels)) {
    return 0;
  }
  return 1;
}
...
int jsd_el1259_PO2SO_config(ecx_contextt* ecx_context, uint16_t slave_id) {
  ...
  // Replace the default (heavy multi-timestamp) PDO image with the minimal
  // per-channel "1x" inputs/outputs that carry just the digital state.
  if (!jsd_el1259_assign_pdos(ecx_context, slave_id,
                              JSD_EL1259_TXPDO_ASSIGN_SDO,
                              JSD_EL1259_DI_PDO_BASE,
                              JSD_EL1259_NUM_DI_CHANNELS)) {
    ERROR("EL1259 slave %u: failed to assign input PDOs", slave_id);
    return 0;
  }
  if (!jsd_el1259_assign_pdos(ecx_context, slave_id,
                              JSD_EL1259_RXPDO_ASSIGN_SDO,
                              JSD_EL1259_DO_PDO_BASE,
